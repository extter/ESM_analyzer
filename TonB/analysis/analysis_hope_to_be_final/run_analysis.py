...
#!/usr/bin/env python3
"""
MSA ANALYSIS FINALE - HEATMAP + CONSERVAZIONE ESM TONB
Heatmap plasma + C_mean/H_mean + Top AA conservati per run
"""
...
CONS_THRESH = 0.8  # Conservazione alta
CONS_CRITICAL = 0.9
...
def analyze_msa(aln_path):
    ...
    for pos in range(L):
        ...
        aa_dominant[pos] = max(counts, key=counts.get)
    ...
    results = {
        ...
        'H_mean': float(-np.nansum([np.sum(row * np.log2(row + 1e-10))
                                   for row in freqs if np.sum(row) > 0]) / (L - gap_cols)),
...
def plot_heatmap(freqs, results):
    ...
    sns.heatmap(freqs[:results['effective_L']].T, 
                cmap='plasma',  # ← PLASMA!
                vmin=0, vmax=1,
...
# =============================================================================
def main():
    print("🔬 MSA ANALYSIS FINALE - PLASMA + CONSERVAZIONE")
    print("-" * 60)
